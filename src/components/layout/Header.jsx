...
export default function Header({ onMenuClick, user }) {
  ...
  const submitSearch = (event) => {
    ...
    if (search.trim()) navigate(`/staff?search=${encodeURIComponent(search.trim())}`);
  };
  ...
  return <header className="sticky top-0 z-30 border-b border-border bg-background/90 backdrop-blur-xl">
    <div className="flex h-16 items-center gap-3 px-4 lg:px-6">
      <button onClick={onMenuClick} className="rounded-lg p-2 hover:bg-muted lg:hidden" aria-label="Open navigation"><Menu className="h-5 w-5" /></button>
      {can('staff.view') && <form onSubmit={submitSearch} className="relative max-w-md flex-1">
      ...
      <div className="ml-auto flex items-center gap-1">
        {canToggleTheme && <button onClick={toggleTheme} className="rounded-lg p-2.5 hover:bg-muted" aria-label="Toggle theme">{theme === 'dark' ? <Sun className="h-5 w-5" /> : <Moon className="h-5 w-5" />}</button>}
        {canViewNotifications && <Link to="/notifications" className="relative rounded-lg p-2.5 hover:bg-muted" aria-label={`${unread} unread notifications`}>
