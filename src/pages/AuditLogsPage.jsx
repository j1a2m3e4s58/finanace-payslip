...
export default function AuditLogs({ embedded = false }) {
  ...
  return <div className="space-y-6">
    ...
    <Card>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2"><CalendarDays className="h-4 w-4 text-primary" />{[['today','Today'],['7days','Last 7 days'],['month','This month'],['year','This year'],['all','All time']].map(([key,label]) => <button key={key} onClick={() => shortcut(key)} className="rounded-full border border-border px-3 py-1.5 text-xs font-semibold hover:border-primary hover:text-primary">{label}</button>)}</div>
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1fr,220px,170px,170px]"><label className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><input aria-label="Search audit logs" className={`${inputClass} pl-9`} value={filters.search} onChange={(event) => setFilters({ ...filters, search: event.target.value })} placeholder="Search users, actions, records or values" /></label><select aria-label="Filter audit logs by action" className={inputClass} value={filters.action} onChange={(event) => setFilters({ ...filters, action: event.target.value })}><option value="">All actions</option>{actions.map((item) => <option key={item} value={item}>{cleanAction(item)}</option>)}</select><input className={inputClass} type="date" aria-label="From date" value={filters.dateFrom} onChange={(event) => setFilters({ ...filters, dateFrom: event.target.value })} /><input className={inputClass} type="date" aria-label="To date" value={filters.dateTo} onChange={(event) => setFilters({ ...filters, dateTo: event.target.value })} /></div>
      <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div className="flex flex-1 flex-wrap gap-2"><input aria-label="Saved audit filter name" className={`${inputClass} max-w-52`} value={filterName} onChange={(event) => setFilterName(event.target.value)} placeholder="Name this filter" /><SecondaryButton onClick={saveFilter} disabled={!filterName.trim()}>Save filter</SecondaryButton><select aria-label="Apply a saved audit filter" className={`${inputClass} max-w-56`} defaultValue="" onChange={(event) => { if (event.target.value) applySavedFilter(Number(event.target.value)); event.target.value = ''; }}><option value="">Saved filters</option>{savedFilters.map((item,index) => <option key={item.name} value={index}>{item.name}</option>)}</select></div><div className="grid grid-cols-2 gap-2 sm:flex"><SecondaryButton onClick={() => { setFilters(defaultFilters); apply(defaultFilters); }}>Clear</SecondaryButton><PrimaryButton disabled={busy === 'load'} onClick={() => { apply(); setFiltersOpen(false); }}><Search className="h-4 w-4" /> Apply filters</PrimaryButton></div></div>
      </div>
    </Card>
