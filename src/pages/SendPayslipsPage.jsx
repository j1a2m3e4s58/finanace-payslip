...
export default function SendPayslipsPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-5 lg:grid-cols-[1.2fr,.8fr]"><Card><label htmlFor="approved-payroll-batch" className="text-xs font-bold uppercase text-muted-foreground">Approved payroll batch</label><select id="approved-payroll-batch" className={`${inputClass} mt-2`} value={selectedId} onChange={(event) => setSelectedId(event.target.value)}><option value="">Select payroll</option>{batches.map((item) => <option key={item.id} value={item.id}>{item.name} · Version {item.version || 1} · {statusLabel(item.status)}</option>)}</select>{batch && <div className="mt-4 grid grid-cols-2 gap-3 rounded-lg bg-primary/[.04] p-4 sm:grid-cols-4"><Info label="Total staff" value={totalStaff} /><Info label="Will receive" value={receiving} tone="green" /><Info label="Blocked" value={blocked} tone={blocked ? 'red' : 'green'} /><div><p className="text-xs text-muted-foreground">Status</p><div className="mt-1"><StatusBadge status={statusLabel(batch.status)} /></div></div></div>}</Card><Card><div className="flex items-center gap-2"><Server className="h-5 w-5 text-primary" /><h2 className="font-heading text-lg font-bold">Email provider</h2></div><p className="mt-3 text-sm font-semibold">{delivery.provider || 'Select a payroll to check SMTP'}</p><p className="mt-2 text-xs text-muted-foreground">Credentials remain on the server and are never returned to this page.</p><div className="mt-3"><StatusBadge status={delivery.mailConfigured ? 'Ready' : 'Not Configured'} /></div></Card></div>
...
function SummaryMetric({ label, value, icon: Icon, tone = 'red' }) { const styles = tone === 'green' ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : tone === 'amber' ? 'bg-amber-500/10 text-amber-800 dark:text-amber-300' : 'bg-red-500/10 text-red-700 dark:text-red-300'; return <div className="rounded-xl border border-border p-4"><span className={`inline-flex rounded-lg p-2 ${styles}`}><Icon className="h-4 w-4" /></span><p className="mt-3 text-2xl font-bold">{value}</p><p className="text-xs text-muted-foreground">{label}</p></div>; }
...
function Info({ label, value, tone = '' }) { return <div><p className="text-xs text-muted-foreground">{label}</p><p className={`mt-1 font-bold ${tone === 'green' ? 'text-emerald-700 dark:text-emerald-300' : tone === 'red' ? 'text-red-700 dark:text-red-300' : ''}`}>{value}</p></div>; }
