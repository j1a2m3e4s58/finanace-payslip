...
import { Archive, CheckCircle2, ChevronLeft, ChevronRight, Download, Eye, FileClock, FileLock2, FileText, History } from 'lucide-react';
...
export default function PayslipPreviewPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card className="overflow-hidden p-0"><div className="flex flex-col gap-2 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><FileText className="h-5 w-5 text-primary" /><div><h2 className="font-heading text-lg font-bold">{selectedEntry ? `${selectedEntry.fullName} — Payslip` : 'Payslip viewer'}</h2><p className="text-xs text-muted-foreground">{selectedEntry ? `${selectedEntry.staffId} · ${selectedEntry.department} · ${selectedEntry.branch}` : 'Select an approved payroll batch.'}</p></div></div>{batch && <StatusBadge status={`Version ${batch.version || 1}`} />}</div><div className="overflow-hidden bg-muted/30 p-2 sm:p-4">{previewUrl ? <div className="mx-auto w-full max-w-[900px] overflow-hidden rounded-lg border border-border bg-white shadow-inner"><iframe title={`Payslip preview for ${selectedEntry?.fullName || 'staff'}`} src={`${previewUrl}#view=FitH&toolbar=1&navpanes=0`} className="h-[65vh] min-h-[480px] w-full border-0 sm:h-[72vh] sm:min-h-[620px]" /></div> : <div className="flex min-h-[420px] flex-col items-center justify-center px-6 text-center"><Eye className="h-10 w-10 text-primary/50" /><h3 className="mt-4 font-heading text-lg font-bold">Preview at readable width</h3><p className="mt-1 max-w-md text-sm text-muted-foreground">Choose a staff member and select Preview. The PDF will fit inside this viewer without making the page scroll sideways.</p></div>}</div></Card>
