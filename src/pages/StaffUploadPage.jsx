  ...
  return <div className="space-y-6">
    ...
    {!rows.length ? <FilePicker busy={busy} chooseFile={chooseFile} /> : <>
      <Card className="sticky top-20 z-20 border-primary/20 bg-card/95 p-3 shadow-lg backdrop-blur-xl sm:p-4">
        ...
        <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{filters.map(([key, label]) => <button key={key} type="button" onClick={() => setFilter(key)} className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-xs font-semibold ${filter === key ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background hover:bg-muted'}`}>{label} <span className="opacity-75">{counts[key]}</span></button>)}</div>
