...
import { Calculator, Check, ClipboardCheck, Copy, Edit3, FileText, Plus, Send, UserRound, X } from 'lucide-react';
...
export default function PayrollBatches() {
  ...
  return <div className="space-y-6">
    ...
    <div className="flex gap-2 overflow-x-auto rounded-xl border border-border bg-card p-2">{statusTabs.map(([key, label]) => <button key={key} type="button" onClick={() => setActiveTab(key)} className={`whitespace-nowrap rounded-lg px-4 py-2 text-sm font-semibold transition ${activeTab === key ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}>{label} <span className={`ml-1 rounded-full px-1.5 py-0.5 text-[10px] ${activeTab === key ? 'bg-white/20' : 'bg-muted'}`}>{counts[key]}</span></button>)}</div>
...
function PayrollProgress({ status }) {
  ...
  return <div><div className="grid grid-cols-5 gap-1">{workflowSteps.map((step, index) => { const complete = current > index; const active = current === index; return <div key={step} className="relative text-center"><span className={`relative z-10 mx-auto flex h-8 w-8 items-center justify-center rounded-full border-2 text-[11px] font-bold ${complete ? 'border-emerald-600 bg-emerald-600 text-white' : active ? 'border-primary bg-primary/10 text-primary' : 'border-border bg-card text-muted-foreground'}`}>{complete ? <Check className="h-4 w-4" /> : index + 1}</span><p className={`mt-1.5 text-[9px] font-semibold sm:text-[11px] ${active ? 'text-primary' : complete ? 'text-emerald-600' : 'text-muted-foreground'}`}>{step}</p>{index < workflowSteps.length - 1 && <span className={`absolute left-[calc(50%+1rem)] right-[calc(-50%+1rem)] top-4 h-0.5 ${current > index ? 'bg-emerald-600' : 'bg-border'}`} />}</div>; })}</div>{['rejected', 'corrected'].includes(status) && <p className="mt-3 rounded-lg bg-amber-500/10 p-2 text-xs font-semibold text-amber-700 dark:text-amber-400">Correction stage: update the returned payroll, then submit it again.</p>}</div>;
}

function CreatePayrollDialog({ form, setForm, copySources, busy, create, close }) {
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-0 sm:items-center sm:p-4"><Card className="w-full max-w-lg rounded-b-none sm:rounded-b-xl"><div className="mb-5 flex items-center justify-between"><div><h2 className="font-heading text-xl font-bold">Create payroll batch</h2><p className="text-sm text-muted-foreground">Only active staff will be included.</p></div><button type="button" onClick={close} className="rounded-lg p-2 hover:bg-muted" aria-label="Close create payroll dialog"><X className="h-5 w-5" /></button></div><form onSubmit={create} className="space-y-4"><label className="block"><span className="mb-1.5 block text-xs font-bold uppercase text-muted-foreground">Payroll month</span><input className={inputClass} type="month" value={form.period} onChange={(event) => setForm({ ...form, period: event.target.value, sourceBatchId: '' })} required /></label><label className="block"><span className="mb-1.5 block text-xs font-bold uppercase text-muted-foreground">Batch name</span><input className={inputClass} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required /></label><label className="block"><span className="mb-1.5 block text-xs font-bold uppercase text-muted-foreground">Copy salary details from</span><select className={inputClass} value={form.sourceBatchId} onChange={(event) => setForm({ ...form, sourceBatchId: event.target.value })}><option value="">Start with empty salary fields</option>{copySources.map((batch) => <option key={batch.id} value={batch.id}>{batch.name} · {batchStatus(batch.status)}</option>)}</select><p className="mt-1 text-xs text-muted-foreground">A confirmation will appear before any salary values are copied.</p></label><div className="flex flex-col gap-2 sm:flex-row"><PrimaryButton disabled={busy} type="submit">{busy ? 'Creating…' : form.sourceBatchId ? 'Review Copy' : 'Create and Enter Salaries'}</PrimaryButton><SecondaryButton type="button" onClick={close}>Cancel</SecondaryButton></div></form></Card></div>;
}
...
function Summary({ label, value, accent }) { return <div className="rounded-lg bg-muted/40 p-3"><p className="text-[10px] uppercase text-muted-foreground">{label}</p><p className={`mt-1 truncate text-sm font-bold ${accent ? 'text-primary' : ''}`} title={String(value)}>{value}</p></div>; }
