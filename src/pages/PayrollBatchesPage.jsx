...
function PayrollProgress({ status }) {
  ...
  return <div><div className="space-y-2 sm:hidden">{workflowSteps.map((step, index) => { const complete = current > index; const active = current === index; return <div key={step} className={`flex items-center gap-3 rounded-lg border p-2.5 ${active ? 'border-primary bg-primary/[.06]' : 'border-border'}`}><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-2 text-[11px] font-bold ${complete ? 'border-emerald-600 bg-emerald-600 text-white' : active ? 'border-primary text-primary' : 'border-border text-muted-foreground'}`}>{complete ? <Check className="h-4 w-4" /> : index + 1}</span><div><p className={`text-xs font-bold ${active ? 'text-primary' : complete ? 'text-emerald-700 dark:text-emerald-400' : 'text-muted-foreground'}`}>{step}</p><p className="text-[10px] text-muted-foreground">{complete ? 'Completed' : active ? 'Current stage' : 'Waiting'}</p></div></div>; })}</div><div className="hidden grid-cols-5 gap-1 sm:grid">{workflowSteps.map((step, index) => { const complete = current > index; const active = current === index; return <div key={step} className="relative text-center"><span className={`relative z-10 mx-auto flex h-8 w-8 items-center justify-center rounded-full border-2 text-[11px] font-bold ${complete ? 'border-emerald-600 bg-emerald-600 text-white' : active ? 'border-primary bg-primary/10 text-primary' : 'border-border bg-card text-muted-foreground'}`}>{complete ? <Check className="h-4 w-4" /> : index + 1}</span><p className={`mt-1.5 text-[11px] font-semibold ${active ? 'text-primary' : complete ? 'text-emerald-700 dark:text-emerald-400' : 'text-muted-foreground'}`}>{step}</p>{index < workflowSteps.length - 1 && <span className={`absolute left-[calc(50%+1rem)] right-[calc(-50%+1rem)] top-4 h-0.5 ${currentStep > index ? 'bg-emerald-600' : 'bg-border'}`} />}</div>; })}</div>{['rejected', 'corrected'].includes(status) && <p className="mt-3 rounded-lg bg-amber-500/10 p-2 text-xs font-semibold text-amber-700 dark:text-amber-400">Correction stage: update the returned payroll, then submit it again.</p>}</div>;
}
