...
function DetailsDrawer({ open, title, columns, row, onClose }) { if (!open || !row) return null; return <div className="fixed inset-0 z-[80] bg-black/45" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="ml-auto h-full w-full max-w-xl overflow-y-auto border-l border-border bg-card p-5 shadow-2xl"><div className="sticky top-0 z-10 mb-5 flex items-center justify-between border-b border-border bg-card pb-4"><div><p className="text-xs font-bold uppercase tracking-widest text-primary">Vertical details</p><h2 className="font-heading text-2xl font-bold">{title}</h2></div><button className="rounded-lg p-2 hover:bg-muted" onClick={onClose}><X className="h-5 w-5" /></button></div><div className="space-y-3">{columns.map((column) => <RecordValue key={column.key} label={column.label} value={row[column.key]} panel />)}</div></aside></div>; }
