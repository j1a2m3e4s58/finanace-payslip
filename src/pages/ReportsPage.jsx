...
export default function Reports() {
  ...
  return <div className="space-y-6">
    ...
    <Card className="sm:hidden"><label className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">Report type<select aria-label="Report type" className={`${inputClass} mt-2`} value={type} onChange={(event) => setType(event.target.value)}>{REPORTS.map(([key,title]) => <option key={key} value={key}>{title}</option>)}</select></label><p className="mt-2 text-xs leading-5 text-muted-foreground">{REPORTS.find(([key]) => key === type)?.[2]}</p></Card>
    ...
    <Card>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><label className="relative xl:col-span-2"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><input aria-label="Search this report" className={`${inputClass} pl-9`} value={filters.search} onChange={(event) => update('search', event.target.value)} placeholder="Search this report" /></label><input className={inputClass} type="date" value={filters.dateFrom} onChange={(event) => update('dateFrom', event.target.value)} aria-label="From date" /><input className={inputClass} type="date" value={filters.dateTo} onChange={(event) => update('dateTo', event.target.value)} aria-label="To date" /><input className={inputClass} type="month" value={filters.month} onChange={(event) => update('month', event.target.value)} aria-label="Payroll month" /><input aria-label="Report year" className={inputClass} type="number" min="2000" max="2100" value={filters.year} onChange={(event) => update('year', event.target.value)} placeholder="Year" /><Select value={filters.department} onChange={(value) => update('department', value)} label="All departments" options={data.options?.departments} /><Select value={filters.branch} onChange={(value) => update('branch', value)} label="All branches" options={data.options?.branches} /><select aria-label="Filter by staff member" className={inputClass} value={filters.staff} onChange={(event) => update('staff', event.target.value)}><option value="">All staff</option>{data.options?.staff?.map((item) => <option key={item.id} value={`${item.staffId} ${item.name}`}>{item.staffId} · {item.name}</option>)}</select></div>
      <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div className="flex flex-1 flex-wrap gap-2"><input aria-label="Saved filter name" className={`${inputClass} max-w-56`} value={saveName} onChange={(event) => setSaveName(event.target.value)} placeholder="Name this filter" /><SecondaryButton onClick={saveCurrentFilter} disabled={!saveName.trim()}><Filter className="h-4 w-4" /> Save filter</SecondaryButton><select aria-label="Apply a saved filter" className={`${inputClass} max-w-60`} defaultValue="" onChange={(event) => { if (event.target.value !== '') applySavedFilter(Number(event.target.value)); event.target.value = ''; }}><option value="">Saved filters</option>{savedFilters.map((item,index) => <option key={`${item.type}-${item.name}`} value={index}>{item.name} · {reportLabel(item.type)}</option>)}</select>{savedFilters.length > 0 && <button className="text-xs font-semibold text-red-600" onClick={() => { const index = savedFilters.findIndex((item) => item.type === type); if (index >= 0) removeSavedFilter(index); }}>Delete first saved filter for this report</button>}</div><div className="grid grid-cols-2 gap-2 sm:flex"><SecondaryButton onClick={() => { setFilters(emptyFilters); apply(emptyFilters); }}>Clear</SecondaryButton><PrimaryButton disabled={busy === 'load'} onClick={() => { apply(); setFiltersOpen(false); }}><Search className="h-4 w-4" /> Apply filters</PrimaryButton></div></div>
      </div>
    </Card>
