...
import { AlertCircle, AlertTriangle, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, ClipboardCheck, Clock3, Cloud, CloudOff, Loader2, Lock, MessageSquareWarning, MoreHorizontal, Save, Send, XCircle } from 'lucide-react';
import { approvePayrollBatch, cancelPayrollBatch, decidePayrollBatch, getPayrollBatch, getPayrollBatches, savePayrollDraft, submitPayrollBatch } from '@/api/portalClient';
...
export function entryIssues(entry, rules = defaultValidationRules) {
  const issues = [];
  ...
  if (manualFields.filter((field) => field !== 'basicSalary').some((field) => Number(entry[field] || 0) > Number(rules.maxOtherAmount ?? 250000)) || Number(entry.basicSalary || 0) > Number(rules.maxBasicSalary ?? 1000000)) issues.push('Unusually large figure');
...
export function PayrollApprovals() {
  ...
  return <div className="space-y-6"><PageHeader title="Payroll Approval" description="Review payroll totals, staff exceptions, and change evidence before payslips can be generated or sent." />
    ...
    <Card><label htmlFor="preview-payroll-batch" className="text-xs font-bold uppercase text-muted-foreground">Payroll batch</label><select id="preview-payroll-batch" className={`${inputClass} mt-2`} value={selectedId} onChange={(event) => setSelectedId(event.target.value)}><option value="">Select payroll</option>{batches.map((item) => <option key={item.id} value={item.id}>{item.name} · Version {item.version || 1} · {batchStatus(item.status)}</option>)}</select></Card>
    ...
    <ResponsiveSheet open={Boolean(decision)} onOpenChange={(next) => !next && setDecision(null)} title={decision === 'approve' ? 'Approve payroll' : decision === 'reject' ? 'Reject payroll' : 'Request correction'} description={decision === 'approve' ? 'Approval unlocks PDF generation and payslip sending.' : 'Finance will see this reason and can correct and resubmit the batch.'}><textarea autoFocus className="min-h-32 w-full rounded-lg border border-border bg-background p-3 text-sm outline-none focus:ring-2 focus:ring-primary/25" value={comments} onChange={(event) => setComments(event.target.value)} placeholder={decision === 'approve' ? 'Optional approval comments' : 'Reason is required'} /><div className="mt-4 grid gap-2 sm:grid-cols-2"><SecondaryButton disabled={busy} onClick={() => setDecision(null)}>Cancel</SecondaryButton><PrimaryButton disabled={busy || (decision !== 'approve' && !comments.trim())} onClick={act}>{busy ? 'Saving…' : 'Confirm decision'}</PrimaryButton></div></ResponsiveSheet>
  </div>;
}
...
export function PayrollEntry() {
  ...
  useEffect(() => {
    ...
    getPayrollBatch(batchId).then((result) => {
      const loaded = result.entries.map((entry) => calculateEntry(entry, result.contributionRates));
      setBatch(result);
      setEntries(loaded);
  ...
  const contributionRates = batch?.contributionRates || defaultContributionRates;
  const validationRules = batch?.payrollValidationRules || defaultValidationRules;
  const activeDeductionFields = useMemo(() => deductionFieldsForRates(contributionRates), [contributionRates]);
  const entryRows = useMemo(() => entries.map((entry) => {
    const changed = entryHasChanges(entry, baselineByStaff[entry.staffRecordId]);
    const issues = entryIssues(entry, validationRules);
    if (changed && !String(entry.changeReason || '').trim()) issues.push('Reason for change required');
    return { entry, changed, issues, fieldIssues: payrollFieldIssues(entry, validationRules) };
  }), [entries, baselineByStaff, validationRules]);
  ...
  const update = useCallback((staffRecordId, field, raw) => {
    setEntries((current) => current.map((entry) => entry.staffRecordId === staffRecordId ? calculateEntry({ ...entry, [field]: field === 'changeReason' ? raw : raw === '' ? null : Number(raw) }, contributionRates) : entry));
    setDirty(true);
  ...
  const persistDraft = useCallback(async (payload, { quiet = false, background = false } = {}) => {
    ...
    try {
      ...
      if (JSON.stringify(entriesRef.current) === payloadSignature) {
        const normalized = saved.entries.map((entry) => calculateEntry(entry, saved.contributionRates));
  ...
  const autoSaveBlocked = useMemo(() => entryRows.some(({ entry, changed }) => !entryCanBeSaved(entry, changed, validationRules)), [entryRows, validationRules]);
  useEffect(() => {
  ...
  return <div className="space-y-6"><PageHeader title={batch.name} description={editable ? `Enter salary figures directly.${batch.sourceBatchName ? ` Copied from ${batch.sourceBatchName}; only explain staff whose values changed.` : ''}` : 'This payroll is read-only because it is in review or has been finalized.'} actions={<><SaveStateIndicator state={saveState} /><StatusBadge status={batchStatus(batch.status)} /><Link to="/payroll/batches"><SecondaryButton>Back to batches</SecondaryButton></Link></>} />
    ...
    <Card>
      ...
      <div className="space-y-3">{pagedEntries.map(({ entry, changed, issues, fieldIssues }) => <PayrollStaffCard key={entry.staffRecordId} entry={entry} editable={editable} update={update} changed={changed} issues={issues} fieldIssues={fieldIssues} deductionFields={activeDeductionFields} expanded={expandedStaffId === entry.staffRecordId} onToggle={() => setExpandedStaffId((current) => current === entry.staffRecordId ? null : entry.staffRecordId)} />)}{!pagedEntries.length && <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">No staff match the selected search and filters.</div>}</div>
    ...
    {editable && <div className="sticky bottom-[calc(4.5rem+env(safe-area-inset-bottom))] z-20 rounded-xl border border-border bg-card/95 p-3 shadow-xl backdrop-blur lg:bottom-3"><div className="flex flex-col gap-3 xl:flex-row xl:items-center"><div className="min-w-0 flex-1"><SaveStateIndicator state={saveState} />{invalid.length ? <p className="mt-1 flex items-center gap-2 text-xs font-semibold text-red-600"><AlertCircle className="h-4 w-4 shrink-0" />{invalid.length} staff record{invalid.length === 1 ? '' : 's'} require attention</p> : <p className="mt-1 flex items-center gap-2 text-xs font-semibold text-emerald-600"><CheckCircle2 className="h-4 w-4" />All payroll entries are ready for submission</p>}</div><div className="grid grid-cols-2 gap-2 sm:hidden"><PrimaryButton className="min-w-0 px-2" disabled={busy || !dirty || autoSaveBlocked || saveState.kind === 'saving'} onClick={() => save(false)}><Save className="h-4 w-4" /> Save Draft</PrimaryButton><SecondaryButton className="min-w-0 px-2" disabled={activeFilteredIndex < 0 || activeFilteredIndex >= filtered.length - 1} onClick={() => moveStaff(1)}>Next Staff <ChevronRight className="h-4 w-4" /></SecondaryButton><button type="button" onClick={() => setMobileActionsOpen(true)} aria-expanded={mobileActionsOpen} className="col-span-2 flex min-h-11 items-center justify-center gap-2 rounded-lg border border-border text-sm font-semibold"><MoreHorizontal className="h-4 w-4" /> More batch actions</button></div><div className="hidden flex-wrap justify-end gap-2 sm:flex"><SecondaryButton disabled={activeFilteredIndex <= 0} onClick={() => moveStaff(-1)}><ChevronLeft className="h-4 w-4" /> Previous Staff</SecondaryButton><SecondaryButton disabled={busy || !dirty || autoSaveBlocked || saveState.kind === 'saving'} onClick={() => save(false)}><Save className="h-4 w-4" /> Save Draft</SecondaryButton><SecondaryButton disabled={activeFilteredIndex < 0 || activeFilteredIndex >= filtered.length - 1} onClick={() => moveStaff(1)}>Next Staff <ChevronRight className="h-4 w-4" /></SecondaryButton><PrimaryButton disabled={busy || invalid.length > 0 || saveState.kind === 'saving'} onClick={submit}><Send className="h-4 w-4" /> Submit</PrimaryButton><button disabled={busy} onClick={() => setCancelReason('')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-red-500/30 px-4 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-500/10"><XCircle className="h-4 w-4" /> Cancel Batch</button></div></div></div>}
    <ResponsiveSheet open={mobileActionsOpen} onOpenChange={setMobileActionsOpen} title="Payroll batch actions" description="Use these actions after reviewing and saving the current staff entry." className="sm:hidden"><div className="grid gap-3"><SecondaryButton disabled={activeFilteredIndex <= 0} onClick={() => { moveStaff(-1); setMobileActionsOpen(false); }}><ChevronLeft className="h-4 w-4" /> Previous Staff</SecondaryButton><PrimaryButton disabled={busy || invalid.length > 0 || saveState.kind === 'saving'} onClick={async () => { setMobileActionsOpen(false); await submit(); }}><Send className="h-4 w-4" /> Submit for Approval</PrimaryButton><Link to="/payroll/batches"><SecondaryButton className="w-full">Back to Payroll Batches</SecondaryButton></Link><button disabled={busy} onClick={() => { setMobileActionsOpen(false); setCancelReason(''); }} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-red-500/30 px-4 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-500/10"><XCircle className="h-4 w-4" /> Cancel Batch</button></div></ResponsiveSheet>
...
function PayrollStaffCard({ entry, editable, update, changed, issues, fieldIssues, deductionFields, expanded, onToggle }) {
...
function payrollFieldIssues(entry, rules = defaultValidationRules) {
  return Object.fromEntries(manualFields.map((field) => { const value = entry[field]; let issue = ''; if (value === null || value === undefined || value === '') issue = 'Enter an amount, including 0.00 when not applicable.'; else if (!Number.isFinite(Number(value))) issue = 'Enter a valid Ghana cedi amount.'; else if (Number(value) < 0) issue = 'Amount cannot be negative.'; else if (field === 'basicSalary' && Number(value) <= 0) issue = 'Basic salary must be greater than GHS 0.00.'; else if (Number(value) > Number(field === 'basicSalary' ? rules.maxBasicSalary ?? 1000000 : rules.maxOtherAmount ?? 250000)) issue = 'This amount is unusually large and must be checked.'; return [field, issue]; }).filter(([, issue]) => issue));
}

function entryCanBeSaved(entry, changed, rules = defaultValidationRules) {
  const amountsAreSafe = manualFields.every((field) => { const value = entry[field]; const maximum = field === 'basicSalary' ? Number(rules.maxBasicSalary ?? 1000000) : Number(rules.maxOtherAmount ?? 250000); return value === null || value === undefined || value === '' || (Number.isFinite(Number(value)) && Number(value) >= 0 && Number(value) <= maximum); });
  return amountsAreSafe && (!changed || Boolean(String(entry.changeReason || '').trim()));
}

function Metric({ label, value, accent = false }) { return <Card className="p-4"><p className="text-xs uppercase text-muted-foreground">{label}</p><p className={`mt-1 text-lg font-bold ${accent ? 'text-primary' : ''}`}>{value}</p></Card>; }
function Summary({ label, value, accent = false }) { return <div><p className="text-xs text-muted-foreground">{label}</p><p className={`font-bold ${accent ? 'text-primary' : ''}`}>{value}</p></div>; }
